...
One medium hardening risk should be considered before broad Phase 11C data-integrity QA: successful manual sync stores `last_sync_at` using the completion timestamp. The five-minute overlap mitigates normal timestamp-edge misses, but a remote write that occurs during a long sync could theoretically be missed on the next pull if the sync duration exceeds the overlap window. A tiny patch is recommended before Phase 11C to store a safer cursor boundary, such as sync-cycle start time or a carefully derived remote high-water mark.
...
|---|---|---|---|---|---|
| 11B-01 | medium | `last_sync_at` cursor | `last_sync_at` advances only after complete convergence success, but the stored value is the completion timestamp. The five-minute overlap is usually enough, but a long sync can still miss remote writes that happen after a table was pulled and more than five minutes before the sync finishes. | `manual-sync.service.ts` calls `getCurrentTimestamp()` after `isCompleteSuccess(result)`. Pull lower bound uses `lastSyncAt - 5 minutes`. | tiny patch recommended |
...
1. Before Phase 11C, consider a tiny cursor patch: record a sync-cycle start timestamp before remote reads and use that as the successful cursor, or otherwise advance to a high-water mark that cannot skip writes that occur during the cycle.
